...
func GuessEncoding(ctx context.Context, d *processor.Document) error {
	...
	if encoding == processor.EncodingUnknown {
		var mimeType string
...
/*
This method examines the first few bytes of the file to determine the encoding from the headers etc

It detects a zstd encoding if the first 4 bytes equal 0xFD2FB528 in little-endian format
as detailed here: https://www.ietf.org/rfc/rfc8878.txt

and bzip2 encoding is identified by the initial three-octet string 0x42, 0x5A, 0x68.
as detailed by: https://www.ietf.org/rfc/rfc5655.txt
*/
