...
func (c *crevParser) Parse(ctx context.Context, doc *processor.Document) error {
	...
	c.getAttestation(doc.Blob, doc.SourceInformation.Source)
	return nil
}

func (c *crevParser) getSubject(statement *in_toto.ProvenanceStatement) {
...
func (c *crevParser) getAttestation(blob []byte, source string) {
	h := sha256.Sum256(blob)
	c.attestations = append(c.attestations, assembler.AttestationNode{
