...
var Package = &assemblerModel.Package{
	Type: "conan",
	Namespaces: []*assemblerModel.PackageNamespace{
		{
			Namespace: "openssl.org",
			Names: []*assemblerModel.PackageName{
				{
					Name: "openssl",
					Versions: []*assemblerModel.PackageVersion{
						{
							Version: "3.0.3",
							Qualifiers: []*assemblerModel.PackageQualifier{
								{
									Key: "distro",
									Value: "debian-11",
								},
...
var PackageNamespaces = []*assemblerModel.PackageNamespace{
	{
		Namespace: "openssl.org",
		Names: []*assemblerModel.PackageName{
			{
		...
		Names: []*assemblerModel.PackageName{
			{
				...
				Versions: []*assemblerModel.PackageVersion{
					{
						Purl:    "pkg:conan/ubuntu/openssl:3.0.3#api?arch=amd64",
						Version: "3.0.3",
						Qualifiers: []*assemblerModel.PackageQualifier{
							{
								Key: "arch",
...
func TestUpdatePurlForNamespaces(t *testing.T) {
	tests := []struct {
		Name        string
		Package     *assemblerModel.Package
		ExpResult   []*assemblerModel.PackageNamespace
		ExpQueryErr bool
	}{
		{
			Name:        "Happy Path",
...
func comparePackageNames(packageNameOne, packageNameTwo []*assemblerModel.PackageName) bool {
	...
	for i := range packageNameOne {
		...
		for j := range packageNameOne[i].Versions {
			if packageNameOne[i].Versions[j].Purl != packageNameTwo[i].Versions[j].Purl {
				fmt.Printf("==%s", packageNameOne[i].Versions[j].Purl)
				fmt.Printf("==%s", packageNameTwo[i].Versions[j].Purl)
				return false
			}
		}
	}
