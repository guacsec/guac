...
func (snq *SourceNameQuery) QueryOccurrences() *IsOccurrenceQuery {
	query := (&IsOccurrenceClient{config: snq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := snq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (snq *SourceNameQuery) WithOccurrences(opts ...func(*IsOccurrenceQuery)) *SourceNameQuery {
	query := (&IsOccurrenceClient{config: snq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (snq *SourceNameQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SourceName, error) {
	...
	if query := snq.withOccurrences; query != nil {
		if err := snq.loadOccurrences(ctx, query, nodes,
			func(n *SourceName) { n.Edges.Occurrences = []*IsOccurrence{} },
			func(n *SourceName, e *IsOccurrence) { n.Edges.Occurrences = append(n.Edges.Occurrences, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (snq *SourceNameQuery) loadOccurrences(ctx context.Context, query *IsOccurrenceQuery, nodes []*SourceName, init func(*SourceName), assign func(*SourceName, *IsOccurrence)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(isoccurrence.FieldSourceID)
	}
